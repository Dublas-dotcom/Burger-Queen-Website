      ...
      <div className="mb-8">
        ...
        <form onSubmit={handleSubmit} className="bg-gray-100 p-4 rounded mb-4 space-y-2">
          <div className="flex flex-col md:flex-row gap-2">
            <input name="name" value={form.name} onChange={handleChange} placeholder="Name" className="p-2 border rounded flex-1" />
            <input name="category" value={form.category} onChange={handleChange} placeholder="Category" className="p-2 border rounded flex-1" />
          </div>
          <div className="flex flex-col md:flex-row gap-2">
            <input name="price" value={form.price} onChange={handleChange} placeholder="Price" type="number" min="0" step="0.01" className="p-2 border rounded flex-1" />
            <input name="image" value={form.image} onChange={handleChange} placeholder="Image URL" className="p-2 border rounded flex-1" />
          </div>
          ...
          <div className="flex gap-2">
            <button type="submit" className="bg-green-600 text-white px-4 py-2 rounded hover:bg-green-700 transition" disabled={loading}>
              {editingId ? 'Update' : 'Add'} Food
            </button>
            {editingId && (
              <button type="button" onClick={handleCancel} className="bg-gray-400 text-white px-4 py-2 rounded hover:bg-gray-500 transition">Cancel</button>
            )}
          </div>
        </form>
        {/* Food items table */}
              ...
              {food.map(item => (
                <tr key={item._id}>
                  ...
                  <td className="p-2 border space-x-2">
                    <button onClick={() => handleEdit(item)} className="bg-blue-600 text-white px-2 py-1 rounded hover:bg-blue-700 transition">Edit</button>
                    <button onClick={() => handleDelete(item._id)} className="bg-red-600 text-white px-2 py-1 rounded hover:bg-red-700 transition">Delete</button>
                  </td>
                </tr>
              ))}
            </tbody>
              ...
              <tbody>
                {filteredOrders.map(order => (
                  <tr key={order._id}>
                    ...
                    <td className="p-2 border font-bold capitalize">{order.status}</td>
                      ...
                      {order.paymentDetails && (order.paymentDetails.last4 || order.paymentDetails.receipt_url) && (
                        <>
                          ...
                          {order.paymentDetails.receipt_url && (
                            <>
                              {' '}|{' '}
                              <a href={order.paymentDetails.receipt_url} target="_blank" rel="noopener noreferrer" className="text-blue-600 underline">Receipt</a>
                            </>
                          )}
                        </>
                      )}
